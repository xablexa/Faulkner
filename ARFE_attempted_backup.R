...
for (i in 1:nrow(LIA_events)) {
  ...
  LIA_events$newEndIndex[i] <- LIA_events$endIndex[i] + LIA_events$combo_full[i]
  LIA_events$combo_add[i] <- LIA_events$combo_add[i] + nrow(combo_ind[LIA_events$endIndex[i] < combo_ind$start & combo_ind$start <= LIA_events$newEndIndex[i],])
}
LIA_events$excerpt_punc <- ""
for (i in 1:nrow(LIA_events)) {
  LIA_events$excerpt_punc[i] <- str_sub(LIA2$text, LIA_events$newStartIndex[i], LIA_events$newEndIndex[i])
}
...
combo_ind$end <- combo_ind$end - 1
#
LIA_events$combo_add <- 0
LIA_events$combo_full <- 0
LIA_events$newStartIndex <- 0
LIA_events$newEndIndex <- 0
LIA_events$newStartIndex[1] <- LIA_events$startIndex[1] + nrow(combo_ind[combo_ind$start <= LIA_events$startIndex[1],])
for (i in 1:nrow(LIA_events)) {
  if (i == 1) {
    LIA_events$combo_add[i] <- nrow(combo_ind[LIA_events$startIndex[i] < combo_ind$start & combo_ind$start <= LIA_events$endIndex[i],]) + nrow(combo_ind[combo_ind$start <= LIA_events$startIndex[i],])
  } else {LIA_events$combo_add[i] <- nrow(combo_ind[LIA_events$startIndex[i] < combo_ind$start & combo_ind$start <= LIA_events$endIndex[i],])#get number of combo for each excerpt
  }
  LIA_events$combo_full[i] <- sum(LIA_events$combo_add[1:i])
  if (i < nrow(LIA_events)) {
    LIA_events$newStartIndex[i + 1] <- LIA_events$startIndex[i + 1] + LIA_events$combo_full[i]
  }
  LIA_events$newEndIndex[i] <- LIA_events$endIndex[i] + LIA_events$combo_full[i]
}
LIA_events$excerpt_punc <- ""
...
LIA_events$excerpt_punc[2]
combo_ind[LIA_events$startIndex[2] < combo_ind$start & combo_ind$start <= LIA_events$endIndex[2],]
combo_ind[LIA_events$newStartIndex[2] < combo_ind$start & combo_ind$start <= LIA_events$newEndIndex[2],]
setwd("C:/Users/Axel/Desktop/Psych Stuff/Faulkner") #set wd
