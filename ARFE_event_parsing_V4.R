...
for (i in 1:nrow(LIA_events)) {
  if (i == 1) {
    ...
    #LIA_events$newEndIndex2[i] <- LIA_events$newEndIndex2[i] + nrow(combo_ind[LIA_events$newEndIndex[i] < combo_ind$start & combo_ind$start <= LIA_events$newEndIndex2[i],])
  } else {
    LIA_events$indLen[i] <- LIA_events$endIndex[i] - LIA_events$startIndex[i] #length of that excerpt
    LIA_events$startIndex[i] <- LIA_events$newEndIndex[i - 1] + 1 #make row's start index the same as previous row's end index + 1
    LIA_events$endIndex[i] <- LIA_events$startIndex[i] + LIA_events$indLen[i] #end index based on length of excerpt
    LIA_events$newEndIndex[i] <- LIA_events$endIndex[i] + nrow(combo_ind[LIA_events$startIndex[i] < combo_ind$start & combo_ind$start <= LIA_events$endIndex[i],]) #find # of rows where combo_ind index is greater than start and less than end (i.e. # of punct in range of excerpt)
    LIA_events$newEndIndex[i] <- LIA_events$newEndIndex[i] + nrow(combo_ind[LIA_events$endIndex[i] < combo_ind$start & combo_ind$start <= LIA_events$newEndIndex[i],]) #same but now index has changed so we want the number of punct between end and new end
    #LIA_events$newEndIndex2[i] <- LIA_events$newEndIndex2[i] + nrow(combo_ind[LIA_events$newEndIndex[i] < combo_ind$start & combo_ind$start <= LIA_events$newEndIndex2[i],])
    if ((LIA_events$newEndIndex[i] - 1) %in% combo_ind$start) {
      LIA_events$newEndIndex[i] <- LIA_events$newEndIndex[i] + 1
    }
    if ((LIA_events$newEndIndex[i] - 1) %in% combo_ind$start) {
      LIA_events$newEndIndex[i] <- LIA_events$newEndIndex[i] + 1
    }
    ...
    if ((LIA_events$newEndIndex[i]) %in% combo_ind$start) {
      LIA_events$newEndIndex[i] <- LIA_events$newEndIndex[i] + 1
    }
  }
}
...
write.xlsx(LIA_events, "ARFE_events.xlsx")
